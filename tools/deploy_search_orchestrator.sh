...
#   CALLBACK_URL, OPEN_SEARCH_REGION (defaults to AWS_REGION),
#   SECRETS_MANAGER_REGION (defaults to AWS_REGION),
...
SECRETS_MANAGER_REGION="${SECRETS_MANAGER_REGION:-${AWS_REGION}}"
...
retry_lambda_update aws lambda update-function-configuration \
  ...
  --timeout "$TIMEOUT" \
  --memory-size "$MEMORY_SIZE" \
  --environment "Variables={CALLBACK_URL=${CALLBACK_URL},OPEN_SEARCH_REGION=${OPEN_SEARCH_REGION},SECRETS_MANAGER_REGION=${SECRETS_MANAGER_REGION},CI_LOOKUP_WORKERS=${CI_LOOKUP_WORKERS},MAX_WORKERS=${MAX_WORKERS},OPENSEARCH_ENDPOINT=${OPENSEARCH_ENDPOINT},OPENSEARCH_CI_INDEX=${OPENSEARCH_CI_INDEX},OPENSEARCH_MAXSIZE=${OPENSEARCH_MAXSIZE},WORKER_LAMBDA_ARN=${WORKER_LAMBDA_ARN},RESULTS_BUCKET=${RESULTS_BUCKET},RESULTS_PREFIX=${RESULTS_PREFIX}}" \
  --region "$AWS_REGION" >/dev/null
