...
FUNCTION_NAME="${FUNCTION_NAME:-rls-search-worker}"
ECR_REPO="${ECR_REPO:-rls-search-worker}"
...
docker buildx build \
  ...
  -f "$ROOT_DIR/lambdas/search_worker/Dockerfile" \
  -t "$IMAGE_URI" \
  -t "$LATEST_IMAGE_URI" \
  "$ROOT_DIR"
...
retry_lambda_update aws lambda update-function-configuration \
  ...
  --timeout "$TIMEOUT" \
  --memory-size "$MEMORY_SIZE" \
  --environment "Variables={OPENSEARCH_ENDPOINT=${OPENSEARCH_ENDPOINT},OPENSEARCH_INDEX=${OPENSEARCH_INDEX},SEMANTIC_OBJECTS_INDEX=${SEMANTIC_OBJECTS_INDEX},OPENSEARCH_CI_INDEX=${OPENSEARCH_CI_INDEX},VERIFIER_MODEL=${VERIFIER_MODEL},RERANKER_CE_MODEL=${RERANKER_CE_MODEL},HF_HOME=/tmp/hf_cache}" \
  --region "$AWS_REGION" >/dev/null
